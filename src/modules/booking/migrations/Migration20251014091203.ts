...
export class Migration20251014091203 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "booking_payment" ("id" text not null, "booking_id" text not null, "stripe_payment_intent_id" text null, "stripe_charge_id" text null, "amount" integer not null, "currency" text not null default 'usd', "status" text check ("status" in ('pending', 'processing', 'succeeded', 'failed', 'cancelled', 'refunded')) not null default 'pending', "payment_type" text check ("payment_type" in ('full', 'deposit')) not null default 'deposit', "customer_email" text not null, "metadata" jsonb null, "refund_amount" integer null, "refund_reason" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "booking_payment_pkey" primary key ("id"));`);
